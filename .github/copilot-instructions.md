...
### Linting
- After modifying any TypeScript file, ensure the code conforms to ESLint rules
- Do not introduce new lint violations
- If linting cannot be verified, state the assumption explicitly

### Type Safety
- All TypeScript code must type-check with no errors
- Avoid `any` unless explicitly required
- Prefer explicit return types for public functions

### Pre-Submission Check
Before finalizing a response involving code changes:
1. Review the code for TypeScript type errors
2. Review for common ESLint violations (unused vars, implicit any, unsafe casts)
3. If potential issues exist, fix them or call them out clearly
